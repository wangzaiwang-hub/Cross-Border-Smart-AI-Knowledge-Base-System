...
| Access Token | 短期 Bearer JWT | 仅规划，签名算法和有效期尚未冻结 |
...
不得在本文中把未冻结的 JWT 算法、Claim、Token 有效期、密码散列参数写成已经实施的事实。它们应在 `BE-0320` 至 `BE-0327` 中通过安全评审、代码和测试冻结。
...
迁移文件一旦进入共享环境禁止修改历史内容，应新增下一版本迁移。生产回滚使用经过评审的前向修复脚本、备份恢复或专门回滚方案，不依赖直接删除历史表。
...
| 2 | Flyway 命名、校验和回滚规范 | `BE-0231` |
| 3 | Redis Key、TTL、锁 owner 校验 | `BE-0232` |
| 4 | MQ Envelope、消费幂等和死信 | `BE-0233` |
| 5 | Gateway、JWT 校验和限流 | `BE-0301` 至 `BE-0309` |
| 6 | Auth 服务、`auth_db` 和完整认证链路 | `BE-0320` 至 `BE-0327` |
| 7 | User、System RBAC 和权限隔离 | `BE-0340` 起及 P09 System |
| 8 | 各业务库 Flyway DDL 和数据字典 | 各业务阶段对应任务 |
