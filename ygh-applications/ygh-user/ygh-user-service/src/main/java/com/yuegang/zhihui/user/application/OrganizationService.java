...
public final class OrganizationService {
    ...
    public EmployeeView createEmployee(CreateEmployeeRequest request){return tx.execute(status->{long id=ids.nextId(),user=positive(request.userId());jdbc.update("INSERT INTO user_employee(id,user_id,employee_no,department_id,employment_status,hired_on) VALUES(?,?,?,?, 'ACTIVE',?)",id,user,request.employeeNo(),optional(request.departmentId()),request.hiredOn());replacePositions(id,request.positionIds());return employee(id);});}
