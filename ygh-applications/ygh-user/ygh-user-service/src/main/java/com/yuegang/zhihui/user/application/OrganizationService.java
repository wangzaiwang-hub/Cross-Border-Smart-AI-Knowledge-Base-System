...
public final class OrganizationService {
    ...
    public DepartmentView createDepartment(CreateDepartmentRequest request){long id=ids.nextId();Long parent=optional(request.parentId());jdbc.update("INSERT INTO user_department(id,parent_id,department_code,department_name,sort_order) VALUES(?,?,?,?,?)",id,parent,request.code(),request.name(),request.sortOrder());return department(id);}
    ...
    public PositionView createPosition(CreatePositionRequest request){long id=ids.nextId();jdbc.update("INSERT INTO user_position(id,position_code,position_name,description) VALUES(?,?,?,?)",id,request.code(),request.name(),request.description());return position(id);}
    ...
    public EmployeeView createEmployee(CreateEmployeeRequest request){return tx.execute(status->{long id=ids.nextId(),user=positive(request.userId());jdbc.update("INSERT IGNORE INTO user_profile(user_id,display_name) VALUES(?,?)",user,"新用户");jdbc.update("INSERT INTO user_employee(id,user_id,employee_no,department_id,employment_status,hired_on) VALUES(?,?,?,?, 'ACTIVE',?)",id,user,request.employeeNo(),optional(request.departmentId()),request.hiredOn());replacePositions(id,request.positionIds());return employee(id);});}
